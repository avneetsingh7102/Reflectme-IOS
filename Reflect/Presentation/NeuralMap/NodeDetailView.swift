import SwiftUI
@preconcurrency import SwiftData

/// Full-screen replacement for the old NodeDetailSheet.
///
/// Per the Reflect Mobile design:
/// - Top bar: back chevron · breadcrumb "Map · N of total" · Insight/Transcript toggle pill.
/// - Session ribbon: dots for every node in the entry, current one enlarged with "HERE" label.
/// - Node head: large emotion disc + serif title + category/emotion pills + metadata line.
/// - Body:
///     * **Insight** tab — AI eyebrow, paragraph, mustard blockquote, connected-nodes chips.
///       Swipe-peek halos hint prev/next nodes.
///     * **Transcript** tab — date-stamped sections with mustard-highlighted lines.
struct NodeDetailView: View {
    let node: SDNode

    @Environment(ServiceContainer.self) private var services
    @Environment(\.modelContext) private var modelContext
    @Environment(\.dismiss) private var dismiss

    enum Tab: Hashable { case insight, transcript }
    @State private var tab: Tab = .insight
    @State private var isLoadingInsight = false

    /// The parent entry — pulled from the inverse relationship. If a node
    /// somehow has no session (it shouldn't), we render a graceful empty.
    private var entry: JournalEntry? { node.session }
    private var siblings: [SDNode] { entry?.mapNodes ?? [node] }
    private var index: Int { siblings.firstIndex(where: { $0.id == node.id }) ?? 0 }
    private var prev: SDNode? { index > 0 ? siblings[index - 1] : nil }
    private var next: SDNode? { index + 1 < siblings.count ? siblings[index + 1] : nil }

    var body: some View {
        ZStack {
            ReflectTheme.canvas.ignoresSafeArea()

            VStack(spacing: 0) {
                topBar
                NodeRibbon(siblings: siblings, currentIndex: index)
                    .padding(.vertical, 6)
                nodeHead
                    .padding(.horizontal, ReflectTheme.edge)
                    .padding(.bottom, 14)

                ScrollView {
                    ZStack(alignment: .top) {
                        switch tab {
                        case .insight:    insightBody
                        case .transcript: transcriptBody
                        }
                    }
                    .padding(.horizontal, ReflectTheme.edge)
                    .padding(.bottom, tab == .insight ? 12 : 28)
                }

                if tab == .insight { askBlock }
            }
        }
        .navigationBarBackButtonHidden(true)
        .task { await loadInsight() }
    }

    // MARK: - Top

    private var topBar: some View {
        HStack(alignment: .center) {
            Button { dismiss() } label: {
                Image(systemName: "chevron.left")
                    .font(.system(size: 16, weight: .semibold))
                    .foregroundStyle(ReflectTheme.ink)
                    .frame(width: 36, height: 36)
                    .background(Circle().fill(Color.black.opacity(0.06)))
            }
            Spacer()
            VStack(spacing: 2) {
                Text("MAP · THIS ENTRY")
                    .font(ReflectTheme.rounded(11, weight: .bold))
                    .tracking(1.4)
                    .foregroundStyle(ReflectTheme.inkFaint)
                Text("\(index + 1) of \(siblings.count)")
                    .font(ReflectTheme.rounded(12, weight: .medium))
                    .foregroundStyle(ReflectTheme.inkSoft)
            }
            Spacer()
            tabToggle
        }
        .padding(.horizontal, 16)
        .padding(.top, 6)
    }

    private var tabToggle: some View {
        Button {
            withAnimation(ReflectTheme.springSnappy) {
                tab = (tab == .insight) ? .transcript : .insight
            }
        } label: {
            HStack(spacing: 5) {
                Image(systemName: tab == .transcript ? "lightbulb" : "text.alignleft")
                    .font(.system(size: 11, weight: .bold))
                Text(tab == .transcript ? "Insight" : "Transcript")
                    .font(ReflectTheme.rounded(12, weight: .semibold))
            }
            .foregroundStyle(tab == .transcript ? Color.white : ReflectTheme.blue500)
            .padding(.horizontal, 11).padding(.vertical, 6)
            .background(
                Capsule().fill(tab == .transcript ? ReflectTheme.blue700 : ReflectTheme.blue50)
            )
        }
        .buttonStyle(.plain)
    }

    // MARK: - Node head

    private var nodeHead: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 14) {
                Circle()
                    .fill(ReflectTheme.color(for: node.emotion))
                    .frame(width: 60, height: 60)
                VStack(alignment: .leading, spacing: 6) {
                    Text(node.label.prefix(1).uppercased() + node.label.dropFirst())
                        .font(.system(size: 26, weight: .medium, design: .serif))
                        .foregroundStyle(ReflectTheme.ink)
                        .lineLimit(2)
                        .minimumScaleFactor(0.8)
                    HStack(spacing: 6) {
                        categoryPill
                        emotionPill
                    }
                }
                Spacer()
            }
            Text(metadataLine)
                .font(ReflectTheme.rounded(12, weight: .medium))
                .foregroundStyle(ReflectTheme.inkFaint)
        }
    }

    private var categoryPill: some View {
        HStack(spacing: 6) {
            Circle().fill(ReflectTheme.blue500).frame(width: 8, height: 8)
            Text(node.category.label)
                .font(ReflectTheme.rounded(11.5, weight: .semibold))
        }
        .padding(.horizontal, 10).padding(.vertical, 6)
        .foregroundStyle(ReflectTheme.blue500)
        .background(Capsule().fill(ReflectTheme.blue50))
    }

    private var emotionPill: some View {
        HStack(spacing: 6) {
            Circle().fill(ReflectTheme.color(for: node.emotion)).frame(width: 8, height: 8)
            Text(node.emotion.label)
                .font(ReflectTheme.rounded(11.5, weight: .semibold))
        }
        .padding(.horizontal, 10).padding(.vertical, 6)
        .foregroundStyle(node.emotion.prefersDarkText ? ReflectTheme.ink : .white)
        .background(Capsule().fill(ReflectTheme.color(for: node.emotion).opacity(0.18)))
    }

    private var metadataLine: String {
        let f = DateFormatter()
        f.dateFormat = "MMM d"
        let date = entry?.date ?? Date()
        return "First mentioned \(f.string(from: date)) · 1 reflection"
    }

    // MARK: - Bodies

    @ViewBuilder
    private var insightBody: some View {
        if isLoadingInsight && (node.expandedContent ?? "").isEmpty {
            SkeletonReflection()
        } else {
            VStack(alignment: .leading, spacing: 14) {
                Text("AI INSIGHT")
                    .eyebrowStyle(color: ReflectTheme.mustard500)

                Text(node.expandedContent ?? fallbackInsight)
                    .font(ReflectTheme.serif(16))
                    .foregroundStyle(ReflectTheme.ink)
                    .lineSpacing(4)

                Text("Connects to \(connectedListString).")
                    .font(ReflectTheme.serif(15))
                    .foregroundStyle(ReflectTheme.inkSoft)
                    .lineSpacing(3)

                if !connectedNodes.isEmpty {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("CONNECTED NODES").eyebrowStyle()
                        FlowLayout(spacing: 8) {
                            ForEach(connectedNodes, id: \.id) { connected in
                                ConnectedNodeChip(node: connected)
                            }
                        }
                    }
                    .padding(.top, 4)
                }
            }
        }
    }

    private var transcriptBody: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack(spacing: 6) {
                Capsule().fill(ReflectTheme.mustard500).frame(width: 22, height: 3)
                Text("underlined = used to build your insight")
                    .font(ReflectTheme.rounded(11.5, weight: .medium))
                    .foregroundStyle(ReflectTheme.inkFaint)
            }

            // We only have one entry-scoped raw transcript today; render it as
            // one date-stamped block of prose.
            VStack(alignment: .leading, spacing: 14) {
                dateSeparator(text: dateStamp(entry?.date ?? Date()))

                Text(transcriptText)
                    .font(ReflectTheme.serif(17))
                    .foregroundStyle(ReflectTheme.ink)
                    .lineSpacing(8)
            }

            HStack(spacing: 8) {
                Image(systemName: "info.circle")
                    .font(.system(size: 12))
                    .foregroundStyle(ReflectTheme.inkFaint)
                Text("Showing this entry's transcript. Cross-entry excerpts will appear once you have more reflections.")
                    .font(ReflectTheme.rounded(12))
                    .foregroundStyle(ReflectTheme.inkFaint)
                    .lineSpacing(2)
            }
            .padding(.top, 16)
            .overlay(
                Rectangle().fill(ReflectTheme.separator).frame(height: 0.5),
                alignment: .top
            )
            .padding(.top, 8)
        }
    }

    private func dateSeparator(text: String) -> some View {
        HStack(spacing: 10) {
            Rectangle().fill(ReflectTheme.separator).frame(height: 0.5)
            Text(text)
                .font(ReflectTheme.mono(10.5))
                .foregroundStyle(ReflectTheme.inkFaint)
                .tracking(0.8)
            Rectangle().fill(ReflectTheme.separator).frame(height: 0.5)
        }
    }

    // MARK: - Ask block (only on insight tab)

    private var askBlock: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top, spacing: 10) {
                Circle()
                    .fill(ReflectTheme.color(for: node.emotion))
                    .frame(width: 26, height: 26)
                VStack(alignment: .leading, spacing: 4) {
                    Text("REFLECT ASKS")
                        .eyebrowStyle(color: ReflectTheme.inkFaint)
                    Text("What does “\(node.label)” feel like when it’s at its very best?")
                        .font(ReflectTheme.serif(15))
                        .foregroundStyle(ReflectTheme.ink)
                        .lineSpacing(3)
                }
                .padding(.horizontal, 14).padding(.vertical, 10)
                .background(
                    RoundedRectangle(cornerRadius: 16, style: .continuous)
                        .fill(ReflectTheme.surface2)
                )
                Spacer()
            }
            HStack {
                Spacer()
                Text("Tap to respond…")
                    .font(ReflectTheme.rounded(13))
                    .italic()
                    .foregroundStyle(.white.opacity(0.75))
                    .padding(.horizontal, 14).padding(.vertical, 10)
                    .background(
                        UnevenRoundedRectangle(
                            cornerRadii: .init(topLeading: 16, bottomLeading: 16,
                                               bottomTrailing: 16, topTrailing: 4),
                            style: .continuous
                        )
                        .fill(ReflectTheme.primary)
                    )
            }
        }
        .padding(.horizontal, 20).padding(.top, 14).padding(.bottom, 22)
        .overlay(Rectangle().fill(ReflectTheme.separator).frame(height: 0.5),
                 alignment: .top)
    }

    // MARK: - Insight loading + helpers

    private var fallbackInsight: String {
        "Tap to ask Reflect to expand on this theme — we’ll quote your own words and offer a gentle question."
    }

    private var transcriptText: String {
        guard let entry else { return "No transcript available." }
        return entry.polishedTranscript.isEmpty ? entry.rawTranscript : entry.polishedTranscript
    }

    private var connectedNodes: [SDNode] {
        guard let entry else { return [] }
        let id = node.id
        let connectedIds = entry.mapLinks
            .filter { $0.source == id || $0.target == id }
            .map { $0.source == id ? $0.target : $0.source }
        return entry.mapNodes.filter { connectedIds.contains($0.id) }
    }

    private var connectedListString: String {
        let labels = connectedNodes.prefix(3).map { $0.label }
        if labels.isEmpty { return "no other nodes yet" }
        if labels.count == 1 { return labels[0] }
        return labels.dropLast().joined(separator: ", ") + " and " + (labels.last ?? "")
    }

    private func dateStamp(_ date: Date) -> String {
        let f = DateFormatter()
        f.dateFormat = "EEE · MMM d · h:mm a"
        return f.string(from: date).uppercased()
    }

    private func loadInsight() async {
        guard (node.expandedContent ?? "").isEmpty else { return }
        isLoadingInsight = true
        let transcript = transcriptText
        do {
            let content = try await services.deepDiveService.expand(
                themeLabel: node.label, transcript: transcript
            )
            let repo = services.makeRepository(context: modelContext)
            try? repo.setExpandedContent(content, for: node)
        } catch {
            print("⚠️ Deep dive failed: \(error)")
        }
        isLoadingInsight = false
    }
}

// MARK: - Subviews

private struct NodeRibbon: View {
    let siblings: [SDNode]
    let currentIndex: Int

    var body: some View {
        HStack(spacing: 14) {
            Spacer(minLength: 0)
            ForEach(Array(siblings.enumerated()), id: \.element.id) { i, n in
                let offset = i - currentIndex
                let isCurrent = offset == 0
                let isNeighbor = abs(offset) == 1
                let size: CGFloat = isCurrent ? 24 : isNeighbor ? 14 : 6
                VStack(spacing: 4) {
                    Circle()
                        .fill(ReflectTheme.color(for: n.emotion))
                        .frame(width: size, height: size)
                        .overlay(
                            Circle()
                                .stroke(ReflectTheme.color(for: n.emotion).opacity(0.22),
                                        lineWidth: isCurrent ? 4 : 0)
                        )
                    if isCurrent {
                        Text("HERE")
                            .font(ReflectTheme.rounded(9, weight: .bold))
                            .foregroundStyle(ReflectTheme.inkSoft)
                            .tracking(0.7)
                    }
                }
                .opacity(abs(offset) > 1 ? 0.3 : 1)
            }
            Spacer(minLength: 0)
        }
        .padding(.horizontal, ReflectTheme.edge)
    }
}

private struct ConnectedNodeChip: View {
    let node: SDNode
    var body: some View {
        HStack(spacing: 8) {
            Circle().fill(ReflectTheme.color(for: node.emotion)).frame(width: 8, height: 8)
            Text(node.label)
                .font(ReflectTheme.rounded(12.5, weight: .medium))
                .foregroundStyle(ReflectTheme.ink)
        }
        .padding(.horizontal, 12).padding(.vertical, 8)
        .background(
            Capsule().fill(Color.white)
                .overlay(Capsule().stroke(ReflectTheme.separator, lineWidth: 1))
        )
    }
}

private struct SkeletonReflection: View {
    @State private var shimmer = false
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("THINKING…")
                .eyebrowStyle(color: ReflectTheme.mustard500)
                .padding(.bottom, 4)
            shimmerBar(width: .infinity)
            shimmerBar(width: 280)
            shimmerBar(width: 220)
            shimmerBar(width: 0).frame(height: 14)
            shimmerBar(width: 200)
            shimmerBar(width: 240)
        }
        .onAppear {
            withAnimation(.linear(duration: 1.6).repeatForever(autoreverses: false)) {
                shimmer.toggle()
            }
        }
    }

    private func shimmerBar(width: CGFloat) -> some View {
        RoundedRectangle(cornerRadius: 6, style: .continuous)
            .fill(
                LinearGradient(
                    colors: [ReflectTheme.surface3, ReflectTheme.surface, ReflectTheme.surface3],
                    startPoint: shimmer ? .leading : .trailing,
                    endPoint: shimmer ? .trailing : .leading
                )
            )
            .frame(maxWidth: width == .infinity ? .infinity : width)
            .frame(height: 12)
    }
}
